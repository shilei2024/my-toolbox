...
class ToolRegistrySyncTest(unittest.TestCase):
    ...
    def test_invoice_tools_are_one_card_with_legacy_route_alias(self) -> None:
        ...
        self.assertNotIn("/tools/invoice-printer/analyze", body)
        self.assertIn("ZIP、PDF、JPG、PNG", body)
        self.assertNotIn("本次可用次数", body)
        self.assertNotIn("使用提示", body)
        self.assertNotIn("只在当前浏览器本地处理", body)
        self.assertNotIn("单个 ZIP 最大", body)
        self.assertIn('data-action="clear-all"', body)
        self.assertIn('data-remove-queue-id=', body)
        self.assertIn('id="ipArchiveErrors"', body)
        self.assertIn('extractArchiveLocally', body)
        self.assertIn('extractZipLevel', body)
        self.assertIn('IP_LOCAL_ZIP_MAX_DEPTH = 8', body)
        self.assertIn('IP_LOCAL_ZIP_MAX_TOTAL_BYTES = 64 * 1024 * 1024', body)
        self.assertIn('IP_LOCAL_ZIP_MAX_RATIO = 100', body)
        self.assertIn('/static/vendor/jszip/jszip-3.10.1.min.js', body)
        self.assertIn('await ensureJSZip()', body)
        self.assertNotIn('requestArchiveBatch', body)
        self.assertNotIn('new FormData()', body)
        self.assertNotIn('base64PDFToFile', body)
        self.assertNotIn("onclick=", body)
        self.assertNotIn("confirm(", body)
        self.assertNotIn(".tif", body)

        jszip_asset = app.test_client().get("/static/vendor/jszip/jszip-3.10.1.min.js")
        self.assertEqual(jszip_asset.status_code, 200)
        self.assertGreater(len(jszip_asset.data), 90_000)
