...
Gallery、Billing、Stripe Test Mode、数据库迁移和页面可以进入预生产验证。完整付费生成上线前仍需补齐 Generation API、Outbox Dispatcher、BullMQ Worker 的生产入口，接通积分 reserve/settle/release，并解决 Flask 与 Next.js 的 `/admin` 路由冲突。

## Go/No-Go 阻断项

- [ ] Generation API、Dispatcher、Worker 有可运行入口和进程守护配置。
- [ ] 前端创建、查询、取消生成任务已接入真实 API。
- [ ] 提交任务在数据库事务内预占积分并写入 outbox。
- [ ] 成功只结算一次；失败或取消只释放一次。
- [ ] `/admin` 仅属于统一管理后台；不得长期保留两个后台。
- [ ] Nginx 已覆盖 Flask、Next、Gallery 和 Billing Webhook 路由。
- [ ] staging 与 production 的数据库、Redis、COS 和支付密钥完全隔离。
- [ ] 备份已在隔离数据库真实恢复。
...
数据库迁移必须按 `0001` 至 `0005` 顺序执行，已有数据库须通过 migration ledger 判断，不得盲目重跑。应用、数据库和对象存储均须保留可核验的备份或版本。
