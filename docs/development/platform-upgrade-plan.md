...
### 2026-08-27 追加：主站 Hero 信息降噪

- **生产影响**：仅调整 Flask 主站首页模板与全局 CSS，不改工具路由、Gallery 跳转、
  登录状态、API、数据库或部署拓扑。
- **信息边界**：Hero 只表达“选择工具并开始处理”的首要任务；AI Gallery 保留为单行
  独立入口，不在主站重复解释提示词、任务、画幅、审核和收藏等 Gallery 内部能力。
- **成本与复用**：沿用现有 Bootstrap Icons 和 CSS 令牌，不增加图片、字体、脚本或依赖；
  紧凑入口可继续复用现有外部应用配置和安全跳转逻辑。
- **响应式与可访问性**：桌面使用单列留白，手机端按钮全宽、Gallery 入口自动换行；保留
  语义标题、键盘焦点和明确的链接标签。
