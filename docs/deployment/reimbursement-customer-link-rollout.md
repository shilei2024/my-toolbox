...
本补丁移除上传/编辑发票弹窗中的“报销状态”，在关联客户旁增加统一客户资料的快捷录入入口，让汇总导出的应酬费客户列只显示简称，并修复火车票触发百度增值税模板 `282103` 后不再继续通用 OCR 的问题。它复用现有客户服务、组织权限、审计与 OCR 配置，不新增数据库迁移、环境变量、依赖或常驻进程，适合按本页现有应用更新流程发布。
