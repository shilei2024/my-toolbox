...
本补丁修复客户项目中推广物料单价和竞品报价在部分浏览器、尤其移动端，无法输入小数的问题。页面不再依赖浏览器的 `number` 控件，而使用文本输入配合小数键盘，并将常见中文小数分隔符规范为 `.`；若移动输入法为一次触控连续插入 `..`，页面会即时折叠为一个 `.`。服务端仍以 `Decimal` 保存，最多允许 5 位有效小数。
