...
2026-09-06 增强继续保持该边界：工作台市场规模通过当前用户的项目权限范围实时派生，品牌/类别下钻不保存第二份金额；主界面采用“客户 → 项目”入口，原项目路由与 API 保持兼容；提醒由腾讯云 systemd timer 调用既有幂等扫描与共享发件箱发送命令，Web/Gunicorn 进程仍不承担调度。

同日的工作台增强在同一派生读模型中增加客户维度，并让到期/停滞摘要成为服务端筛选入口。筛选条件和数量使用同一组 SQL 条件，项目更新后不维护计数缓存，下一次工作台请求直接基于 PostgreSQL 当前记录重算，避免摘要与项目详情不一致。
