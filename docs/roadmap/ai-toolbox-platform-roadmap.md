...
| 里程碑 | 交付内容 | 依赖 | 风险 |
| --- | --- | --- | --- |
| M1 生产闭环 | Generation API、Dispatcher/Worker 入口、积分原子预占/结算、统一 Admin 路由 | Phase 1–10 | 已上线生图闭环（含 M1.1–M1.5）；待 M4 视频通过 Staging/生产门禁 |
| M2 国内支付 | Payment Provider 契约固化、微信支付、支付宝、退款与对账、订阅会员与到期自动降级、兑换码购买页（合并进 /pricing） | M1 | 商户资质、回调幂等、财务合规 |
| M3 平台任务中心 | 通用 Task Schema、跨模块历史、通知、成本与 SLA | M1 | 已交付生成来源任务中心与统一队列观测；多来源接入按 ADR-0020 扩展 |
| M4 AI 视频/OCR | 复用任务、积分、存储、Provider Registry 接入两个新模块 | M3 | 视频链路（Ark + 本机 ComfyUI）已实现并本地验证，待 Staging 验收后启用；OCR 待开展 |
| M5 可观测与增长 | 统一指标、告警、配额、优惠券、反馈、SEO 增长、Provider 凭证后台维护与新 Provider/模型可视化新增 | M2–M4 | 指标隐私、促销滥用 |
