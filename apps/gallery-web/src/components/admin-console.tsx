...
function ModerationPanel({ items, busy, onModerate, onDelete }: { readonly items: readonly AdminImageItem[]; readonly busy?: string; readonly onModerate: (item: AdminImageItem, decision: "approved" | "rejected") => void; readonly onDelete: (item: AdminImageItem) => void }) {
  ...
  return <div className="moderation-list">{items.map((item) => <article className="moderation-card" key={item.id}><a className="moderation-thumb" href={`/gallery/${item.slug}`} target="_blank" rel="noreferrer">{item.mediaType === "video" ? <video src={`${item.thumbnailUrl}#t=0.1`} muted playsInline preload="metadata" aria-label={item.title || "待审核 AI 视频"} onMouseEnter={(event) => { void event.currentTarget.play().catch(() => undefined); }} onMouseLeave={(event) => { const video = event.currentTarget; video.pause(); video.currentTime = 0.1; }} /> : <img src={item.thumbnailUrl} alt={item.title || "待审核 AI 作品"} />}{item.mediaType === "video" && <span className="media-badge video">视频</span>}</a><div className="moderation-copy"><div className="admin-row-title"><div><p>{item.title || "未命名作品"}</p><span>{item.mediaType === "video" ? "视频 · " : ""}{item.workflowName} · {formatDate(item.createdAt)}</span></div><Status value={item.moderationStatus} /></div><dl className="admin-inline-meta"><div><dt>{item.mediaType === "video" ? "视频" : "图片"}</dt><dd>{item.visibility}</dd></div><div><dt>Prompt</dt><dd>{item.promptVisibility}</dd></div><div><dt>Slug</dt><dd>{item.slug}</dd></div></dl><div className="admin-row-actions"><button className="button primary" type="button" disabled={busy === `image:${item.id}`} onClick={() => onModerate(item, "approved")}>批准公开</button><button className="button" type="button" disabled={busy === `image:${item.id}`} onClick={() => onModerate(item, "rejected")}>拒绝</button><button className="button danger" type="button" disabled={busy === `image:${item.id}`} onClick={() => onDelete(item)}>删除</button></div></div></article>)}</div>;
}
...
function WorkflowPanel({ items, busy, onSubmit }: { readonly items: readonly AdminWorkflowItem[]; readonly busy?: string; readonly onSubmit: (item: AdminWorkflowItem, event: FormEvent<HTMLFormElement>) => void }) {
  ...
  return <div className="workflow-admin-grid">{items.map((item) => <form className="workflow-admin-card" key={item.id} onSubmit={(event) => onSubmit(item, event)}><div className="admin-row-title"><div><p>{item.name}</p><span>{item.slug} · {item.category} · {item.mode === "api" ? "API 模型" : "工作流"}</span></div><Status value={item.isEnabled ? "enabled" : "disabled"} /></div><div className="workflow-version"><span>Active version</span><strong>{item.activeVersion ? `v${item.activeVersion}` : "未发布"}</strong><small>{item.bindingCount} 个可用 binding</small></div><div className="workflow-controls"><label><input name="isEnabled" type="checkbox" defaultChecked={item.isEnabled} />允许用户选择</label><label>排序<input name="sortOrder" type="number" min="0" max="10000" defaultValue={item.sortOrder} /></label><button className="button" type="submit" disabled={busy === `workflow:${item.id}`}>保存工作流</button></div></form>)}</div>;
}
