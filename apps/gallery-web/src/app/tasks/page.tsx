...
function TaskHeader() { return <section className="collection-head"><p className="eyebrow"><span className="eyebrow-dot" />Task center</p><h1 className="collection-title">任务中心</h1><p className="collection-copy">集中查看图片、视频及后续异步工具的状态、积分结算与输出。</p></section>; }
function TaskRow({ task }: { readonly task: TaskSummary }) {
  const output = task.outputLinks[0];
  const outputHref = output?.mediaType === "image" ? `/gallery/${output.slug}` : output?.mediaType === "video" ? output.url : undefined;
  return <article className="task-center-item"><span className={`task-indicator ${task.status}`} aria-hidden="true" /><div className="task-center-main"><div><span className="task-module">{task.module === "generation" ? (task.mediaType === "video" ? "视频生成" : "图片生成") : task.module}</span><strong>{task.title}</strong></div><small>任务 {task.sourceId.slice(0, 8)} · 创建于 {formatTime(task.createdAt)} · {statusLabel(task.status, task.cancelRequested)}</small>{task.error ? <small className="task-error">{task.error.message}</small> : null}</div><div className="task-center-cost"><span>{task.status === "completed" ? "已结算" : "预占"}</span><strong>{task.status === "completed" ? task.creditsCharged : task.creditsReserved} 积分</strong></div>{outputHref ? <Link className="button task-output" href={outputHref} target={output?.mediaType === "video" ? "_blank" : undefined} rel={output?.mediaType === "video" ? "noopener noreferrer" : undefined}>查看结果</Link> : <Link className="button task-output" href="/create">创作台</Link>}</article>;
}
