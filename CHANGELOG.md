...
## 0.9.44 · 量产状态与推广物料体验优化 / 2026-09-07

- 客户项目新建页为业务经理/组织管理员增加“量产”状态，并保留服务端权限校验。
- 项目转入量产只需填写变更原因，物料、实际量产日期和结果说明不再作为阻断条件。
- 推广物料品牌选择复用当前用户报销助手的品牌目录，只展示品牌名称并保存名称快照。
- 无数据的物料机会分类卡片不再显示；初始状态只保留添加推广物料入口。

## 0.9.43 · 客户市场规模与提醒项目下钻 / 2026-09-06

- 工作台在整体 TAM/SAM/SOM 下增加按客户汇总的 TAM/SAM/SOM，沿用授权项目范围、非归档口径和客户简称，点击客户可进入其项目上下文。
- 已逾期、今日到期、7 天内到期、长期未更新四个提醒卡片支持筛选下钻；项目结果直接进入详情更新，工作台禁用响应缓存并在下一次访问时实时重算数量与清单。
- 缩短工作台头图区、提升提醒卡片可点击状态和移动端客户规模布局；无数据库迁移、无新增依赖、无新增后台进程。

## 0.9.42 · 客户项目市场工作台与真实邮件调度 / 2026-09-06

- “我的工作台”汇总当前用户可见、非归档项目的 TAM/SAM/SOM，并提供品牌、物料类别下钻；完全复用项目级计算口径，不新增冗余金额字段。
- 主操作路径调整为“客户与项目”：客户列表进入客户详情后再选择或新建项目；保留原项目 URL、查询页与 API 兼容已有书签和集成。
- 邮件提醒新增脱敏配置自检、同生产路径的测试邮件命令、上海时区邮件内容，以及腾讯云 systemd service/timer，每 10 分钟幂等扫描并投递。
- 本次无数据库迁移、无新增第三方依赖；关闭 timer 和两个提醒开关即可停止新扫描及真实发信。

## 0.9.41 · 发票录入与应酬明细客户显示优化 / 2026-09-06

- 上传/编辑发票弹窗移除“报销状态”字段；新发票继续由服务端安全默认为“待报销”，编辑历史发票不会因字段隐藏而重置原状态。
- 关联客户旁新增受组织权限约束的快捷录入入口，复用统一客户服务与审计记录；保存后自动加入下拉并选中，不新增发票专属客户数据。
- 汇总导出的应酬费明细客户选项及 Excel 内容只显示客户简称（简称为空时回退客户全称），不再拼接客户等级。
- 修复火车票先经过增值税模板识别时，百度 `282103` 直接打断通用 OCR 降级的问题；通用识别补齐火车票票号、日期、票价和摘要回填，前端不再暴露百度/腾讯内部错误码。
- CI 安全门禁同步刷新 Generation Service 锁文件中的 `fast-uri`、`fastify`、`qs` 现有间接依赖，`npm audit --omit=dev --audit-level=high` 恢复为零漏洞；不新增业务依赖。
- 本次无数据库迁移、无新增环境变量或常驻进程；回滚只需恢复上一应用版本。

## 0.9.40 · 管理日志中国标准时间校准 / 2026-09-01

- 修复 PostgreSQL 会话时区参与无时区字段写入、导致日志时间可能被重复偏移的问题：旧版无时区字段在写入前统一规范化为 UTC wall time。
- 中国时间展示改用 IANA `Asia/Shanghai`，管理日志明确标注 `UTC+8`，进程日志明确输出 `+0800`；数据库仍保存 UTC，不改变日期筛选口径。
- 新增日志写入与页面显示回归测试、ADR-0036，以及无数据库迁移的腾讯云更新与回滚说明。

## 0.9.39 · 报销客户关联与曜石主题可读性 / 2026-08-31

- 补齐曜石专业对表单、下拉、只读控件、表格、弹窗、警告、分页及业务模块语义表面的高对比样式，修复固定浅色背景与继承浅色文字叠加导致的不可读问题。
- 客户等级统一为 `0-1/A/AA/AAA`；客户项目列表优先显示简称，项目自身 A/B/C/D 项目等级保持不变。
- 报销发票与招待/出差明细可关联同组织客户主数据，显示简称并实时读取客户等级；匿名和历史未关联报销保持兼容。
- 新增向后兼容迁移 `d3e4f5a6b7c8`、ADR-0035、回归测试和腾讯云发布/验收/回滚命令。

## 0.9.38 · 主站工作台信息架构与主题升级 / 2026-08-30

- 将授权可见的“客户项目工作台”首页卡片并入“业务工具”分类，保持原功能开关、权限校验和 `/customer-projects/` 路由不变；分类计数同步包含该业务入口。
- 主站升级为更强调层次和工作效率的工具工作台视觉，保留直接进入工具库的首页结构，不新增营销 Hero 或业务接口。
- 新增默认“曜石专业”、GitHub 风格和明亮风格三种界面；主题使用浏览器本地偏好保存，不新增用户字段、数据库迁移、环境变量或外部依赖。主题令牌可复用给后续平台模块，异常时安全回退默认主题。
...
## 0.9.35 · 机会分类徽章快速转换 / 2026-08-29

- 物料机会徽章（Design In / Design Win / Evaluation / Lost）支持点击直接转换：弹出四类单选弹窗，一步完成类型切换
- Lost 转出时弹窗自动出现推广品牌/型号补充输入（服务端规则前端化）
- 快速转换表单回填主推/型号待确认状态，避免路由默认值清空标记
- 转换沿用既有 material_update_commercial 端点：乐观锁、审计日志、Lost 转出服务端校验全部生效

## 0.9.34 · 交互与命名优化 / 2026-08-28

- 客户项目导航和关键操作图标升级为统一的高对比语义图标容器，加入克制微动效与减少动态效果兼容；物料标记同步升级。
- 汇率预览改为独立 USD/CNY 信息条，默认主题强制使用深色高对比币别文字，避免继承全局浅色 `text-muted` 后不可读。
- 修复添加推广物料单价无法录入小数：价格输入不再依赖浏览器 `number` 控件，统一使用小数文本键盘并规范化常见中文小数分隔符；服务端继续限制最多 5 位小数，增加页面表单小数写入回归测试。
- 修复部分移动输入法一次小数键触控出现 `..`：客户端仅将相邻分隔符折叠为一个 `.`，非连续的非法格式仍由服务端拒绝。
- 修复单机数量/项目年用量空值被默认填 0：保持“单机数量（PCS，整数）”占位符提示。
- 项目页“评估”统一更名为“项目等级”（含新建、编辑、导入导出表头，导入兼容旧表头“评估等级”）。
- 阶段徽章（评估/送样/小批等）可点击弹窗变更阶段，转化记录写入跟进时间线（沿用既有 ProjectStageEvent）。
- “主业务”统一更名为“主负责人”（列表、详情、新建、重新激活、导入导出与错误提示，导入兼容旧表头“主业务邮箱”）。
...
- 跟进时间线新增独立留言及同组织成员 @ 记录；留言不改变项目版本、下一步或提醒时间，审计仅保存长度和提及人数。
- 点击项目名称可直接打开基础信息编辑弹窗，继续使用现有权限、CSRF 与乐观锁保护。
- 新增只加表迁移 `b9c0d1e2f3a4`、API/页面回归测试、ADR 和生产发布回滚手册。

## 0.9.31 · 客户项目物料机会分类与市场规模 / 2026-08-28

- 推广物料按设计中、已匹配料号机会和竞品替代机会三类展示；点击推广型号或竞品型号即可在弹窗中编辑，继续使用版本锁、权限和审计。
- 项目年用量只接受正整数并以 PCS 展示；历史 Excel 导入同步执行整数校验。
- 新增按“项目年用量 × 单机数量 × 美金单价”实时派生的单条年度机会金额及 TAM/SAM/SOM，不重复存储计算结果。
- Excel 导出增加机会分类和年度机会金额；新增只加列迁移 `a8b9c0d1e2f3`、ADR、staging 验收和回滚手册。

## 0.9.30 · 客户项目 Phase 4 保存与组织共享视图 / 2026-08-28

- 用户可保存当前项目关键字/阶段筛选为个人视图；个人命名空间按组织与用户隔离。
- 组织管理员可发布或删除组织共享视图，同组织成员可使用但不能越权管理。
- 保存内容只接受筛选白名单，不保存 SQL、URL 或公开访问令牌；创建和删除均写入安全审计。
- 新增只加表迁移 `f7a8b9c0d1e2`，并更新腾讯云服务器迁移、试点验证和回滚指导。

## 0.9.29 · 客户项目 Phase 4 受控导出 / 2026-08-28

- 将早期项目 Excel 导出升级为组织级策略控制：默认仅组织管理员和业务经理可用，可配置允许角色、价格列、项目数与输出行数上限。
- 导出严格复用组织与项目成员可见范围，不包含联系人电话/邮箱；不具备价格权限或策略关闭时不生成价格列。
- 新增 Excel 公式注入防护、成功文件 SHA-256 审计、筛选范围审计和超限拒绝审计。
- 使用 write-only XLSX 生成降低内存占用，并新增只加表迁移 `e6f7a8b9c0d1`。

## 0.9.28 · 客户项目 Phase 4 工作日日历与受控导入 / 2026-08-28
- 新增共享组织工作日日历，可把普通工作日覆盖为休息日、把周末覆盖为调休工作日；提醒提前日、逾期日和经理升级日统一使用该日历。后台变更日历或组织提醒策略会取消未发送旧意图并推进策略版本。
- 新增经理可用的历史 XLSX 导入闭环：5MB/1000 行/解压预算限制、固定模板字段识别、逐行预览校验、只提交有效行、批次与行级审计，以及仅撤销导入后未修改记录。原始文件不落盘，错误行不写业务表。
- 新增只加表 migrations `c4d5e6f7a8b9` 与 `d5e6f7a8b9c0`；不新增队列、对象存储或实时推送。附件和共享视图仍需真实需求与权限模型确认后进入后续切片。

## 0.9.27 · 客户项目 Phase 3 生命周期闭环 / 2026-08-28
- 新增经理审批的暂停/终态项目重新激活流程，原阶段事件、失败复盘和活动历史保持不变；负责人、下一步、跟进时间和主业务成员标记在同一事务更新，并继续使用版本锁与幂等键。
- 新增衍生项目页面/API：生成独立项目编号和生命周期，记录来源链接，可选择复制有效成员、物料与竞品；活动和阶段历史不复制，价格沿用可复现快照，不重新请求汇率。
- 新增生命周期页面与 JSON 报表，沿用项目可见范围，返回生成时间、数据时点、筛选和统计定义；当前状态统计明确不是转化率。本阶段无 migration、外部服务或新生产配置，功能继续受客户项目总开关和试点范围保护。

## 0.9.26 · 客户项目提醒覆盖与成员偏好 / 2026-08-28
- 项目可单独启停提醒，并对 PM/FAE 抄送选择继承组织、强制开启或关闭；项目成员可关闭个人邮件接收，主业务和升级经理仍按职责与有效账号规则确定。
- 覆盖策略和成员偏好变化会取消未发送旧提醒；项目覆盖版本进入幂等键，下一次扫描按新收件范围生成意图，避免旧配置继续发送。
- 新增只加表 migration `b3c4d5e6f7a8` 和项目详情管理入口；组织节假日日历仍按路线图留在 Phase 4，本阶段继续明确使用普通周历。

## 0.9.25 · 客户项目 Phase 2 提醒闭环首切片 / 2026-08-28
- 新增组织级提醒策略、到期前/到期日/逾期及分级停滞计算；重复扫描使用稳定幂等键，停用账号不会进入收件人，有效跟进、时间变化、暂停、终态和软删除会取消未发送旧提醒。
- 新增可复用通知发件箱、逐收件人投递记录、dry-run/SMTP 适配器、退避重试、死信、过期领取回收和扫描/发送心跳；统一后台可维护策略并查看安全通知状态，不记录邮件正文或供应商原始错误。
- 新增只加表 migration `a2b3c4d5e6f7`、外部 cron 可调用的幂等 CLI、三重生产开关和初学者部署/回滚手册；默认不扫描且不真实发信，不新增 Redis、常驻提醒线程或独立服务。

## 0.9.24 · 客户项目物料与竞品维护闭环 / 2026-08-28
- 推广物料现可在项目详情页及 Customer Projects API 中编辑品牌、型号、品类、应用位置、单机数量、主推状态和商务字段；竞争方案支持品牌、型号、渠道、在用状态、报价与观察日期编辑。
- 物料和竞争方案更新均使用对象级版本号与原子条件更新，过期版本返回 409，不静默覆盖；删除要求原因并采用软删除，列表与详情隐藏已删除记录但保留历史和脱敏审计。
- 复用现有组织 RBAC、项目可见范围、功能开关和 PostgreSQL 事实源，不新增数据库迁移、外部服务或生产配置；应用回滚可直接回退代码并继续保留既有业务记录。

## 0.9.23 · 客户项目商务台账增强 / 2026-08-27
- 项目新增产品名称与项目年用量；客户台账补齐 A/B/C/D 评级创建和修改能力。
- 推广物料新增单机数量、美元单价、13% 增值税含税人民币单价及 USD/CNY 汇率快照；复用主站汇率服务，录入 USD 或含税 CNY 任一币别即可折算，历史价格不随之后的汇率变化漂移。
- 单价写入仅允许组织管理员、业务经理、业务和 PM，FAE 对价格保持只读但可维护单机数量；新增物料级乐观锁、脱敏审计及权限回归。
- 项目列表新增按当前筛选范围导出 Excel，导出行为写入审计；新增只加列 migration `f1a2b3c4d5e6`，旧项目字段允许暂为空并在编辑时补齐。

## 0.9.22 · 客户项目跟进 Phase 1 核心台账 / 2026-08-27
- 在 Flask 主站新增独立客户项目领域：组织/成员角色、客户/联系人、项目、项目成员、推广物料、竞争方案、追加式跟进、阶段历史、脱敏审计和软删除/恢复；统一后台负责组织初始化与角色维护。
- 新增响应式工作台、项目筛选、客户录入、新建/详情、移动端快速跟进、物料/竞品、成员和回收站页面；项目写入使用幂等键与版本号，冲突返回 409 而不覆盖他人内容。
- 引入 Flask-Migrate/Alembic 和只新增对象的 migration `8904db6a3fa5`；功能开关默认关闭，提供可选试点邮箱、CLI/后台初始化、部署回滚和运维文档。SQLite 兼容迁移及 8 项领域/API/页面测试通过；真实 PostgreSQL staging、性能、备份恢复和生产发布仍是阻断项。
- 补齐项目基础信息编辑、组织时区工作台日期边界、项目详情统一时间线及高风险写入的组织隔离校验；新增物料/竞品、阶段流转、检索和响应式人工验收记录。当前客户项目测试 11 项、全量测试 145 项通过；真实 PostgreSQL staging、性能、备份恢复和生产发布仍是阻断项。
- 修复主站首页客户项目入口：全局权限上下文正确注入，授权试点用户会看到“客户项目工作台”卡片；未授权用户继续隐藏入口并由服务端拒绝访问。

## 0.9.21 · 客户项目跟进 Phase 0 架构冻结 / 2026-08-27
- 基于客户项目跟进 PRD 冻结主站模块化单体、PostgreSQL 事实源、共享组织/RBAC/审计/通知边界、功能开关与试点发布策略；不新建 CRM 服务、Redis、WebSocket 或独立后台。
- 新增状态机、数据字典、乐观锁/幂等事务、Customer Projects API v1、桌面/移动端低保真原型，以及正式迁移、预生产演练和保留数据的回滚计划。
- 本阶段仅交付设计与实施契约，不新增路由、数据库表、依赖或生产配置；Phase 1 仍受业务冻结项确认、正式迁移实现、PostgreSQL 集成和权限/回滚验收门禁约束。

## 0.9.20 · 发票工作台说明清理 / 2026-08-27
- 移除发票工具右侧“可用次数”和旧“使用提示”侧栏，避免本地提取上线后继续展示服务器解压、批次门限等过时说明。
- 移除卡片内重复工具标题、隐私/排障说明和冗长格式描述；页面只保留文件选择、处理、清单、打印、下载、删除、导出及必要错误提示。
- 发票工作台改为全宽布局；本地递归 ZIP、安全门限、清单和打印导出功能保持不变，其他工具仍保留原有侧栏。

## 0.9.19 · 发票 ZIP 全本地提取 / 2026-08-27
- “发票提取与批量打印”改为在浏览器内递归解开 ZIP 并直接生成 PDF 文件，不再上传 Flask 或返回 Base64 JSON，从链路上移除 Cloudflare HTTP 524、请求体门限和重复服务端任务。
- 浏览器端保持最深 8 层、单成员 32MB、单包累计 64MB、压缩比 100 的安全边界；兼容 Windows 路径、无扩展名 ZIP/PDF、内容摘要去重以及损坏/加密包诊断。
- 最多两个顶层 ZIP 并发处理，结果仍按选择顺序加入工作台；JSZip 3.10.1 与许可证作为主站静态资源发布，提取与导出复用同一个可重试加载器，两个固定版本 CDN 仅作降级。
- 历史分析接口继续保留用于兼容和快速回滚；当前页面不再调用接口，不新增数据库、生产依赖、云资源或部署配置。

## 0.9.18 · 发票多压缩包 524 与解压性能优化 / 2026-08-27
- 同一批最多处理两个顶层 ZIP，并在服务端固定双线程并行解压，将两个压缩包的总等待时间从串行相加缩短到接近较慢单包的耗时；保持一次请求、一次用量计量。
- 仅对无扩展名成员执行文件签名识别，不再逐个打开 XML、OFD、图片等明确无关的辅助文件，减少大清单发票包的重复解压和 CPU 消耗。
- 返回结果先按内容去重并检查响应预算，只对最终会返回的 PDF 执行 Base64 编码，避免为被 48MB 响应上限截断的文件消耗 CPU 和内存。
- HTTP 524 不自动重发，避免边缘入口已经超时但 Flask 仍在处理时产生重复任务；补充双包并发、无关成员跳过和页面批量上限回归测试。

## 0.9.17 · 发票压缩包请求诊断与逐包降级 / 2026-08-27
- 统一发票工作台的 ZIP 请求显式要求 JSON 响应，配额、CSRF、上传门限和服务错误不再被登录页或 HTML 错误页吞掉；失败原因会持续显示在页面和对应待处理文件下。
- 多 ZIP 整批受到网关或网络错误影响时自动逐包重试，小压缩包不再被同批失败连带阻断；失败项目继续保留，可直接再次处理或删除。
- 后端返回逐压缩包递归诊断，包括内层包数量、损坏/无效 ZIP、加密或读取异常；新增无效 ZIP 与递归诊断回归测试，不降低原有解压安全限制。
