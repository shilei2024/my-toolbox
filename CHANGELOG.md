...
## 0.9.14 · 主站首页 Hero 精简 / 2026-08-27
- 首页首屏收敛为“常用工具，打开就能用”的单一任务，移除右侧能力示意卡、英文标签和
  类似提示词的宣传文案，减少重复入口与无关说明。
- 将原大型 AI 创作宣传区替换为紧凑的 AI Gallery 入口，不再展示画幅、任务、审核等
  功能清单；工具库、登录状态和 Gallery 跳转能力保持不变。
- 同步优化桌面、平板和手机布局；不新增图片、依赖、API、数据库或部署配置，回滚只需
  恢复首页模板和全局样式。
