...
export class ImagePersistenceService {
  ...
  async persist(jobId: string, outputs: readonly ProviderImageOutput[]): Promise<readonly StoredAsset[]> {
    ...
    try {
      ...
      for (const [index, output] of outputs.entries()) {
        ...
        try {
          ...
          assets.push(await this.#storage.upload({ objectKey: key, body: createReadStream(local), contentType: output.mimeType, contentLength: info.size, metadata: { job_id: jobId, output_index: String(index) } }));
