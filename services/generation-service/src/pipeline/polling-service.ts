...
export class PollingService {
  ...
  async wait(provider: ImageProvider, submission: ProviderSubmission, context: ProviderCallContext, timeoutMs?: number, timeoutRetryable = true): Promise<ProviderStatusResult> {
    ...
    for (let attempt = 0; attempt < this.#options.maxAttempts; attempt += 1) {
      if (deadline !== undefined && Date.now() >= deadline) throw new ProviderError({ providerCode: provider.descriptor.code, category: "timeout", code: "polling_exhausted", message: "Provider polling timed out", retryable: timeoutRetryable, externalRequestId: submission.externalRequestId });
